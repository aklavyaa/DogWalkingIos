...
extension OwnerSignUpVC {
    ...
    func createAccount(name: String, email: String, age: String, password: String, address: String,description:String, profile: String) {
        ...
        {  err in
            if let err = err {
                ...
                GFunction.userOwner = UserOwnerModel(docID: ref!.documentID, name: name, email: email, password: password, userType: UserType.Owner.getname(), isEnable: true, description: description, profile: profile, address: address, age: age,isReserved: false)
    ...
    func getExistingUser(name: String, email: String, age: String, password: String, address: String,description:String, profile: String) {
        ...
        _ = AppDelegate.shared.database.collection(dDogOwner).whereField(dUser_email, isEqualTo: email).addSnapshotListener{ querySnapshot, error in
            ...
            if snapshot.documents.count == 0 {
                self.createAccount(name: name, email: email, age: age, password: password, address: address,description:description, profile: profile)
                self.flag = true
            }else{
                if !self.flag {
    ...
    func uploadImagePic(img1 :UIImage, name: String, email:String, age:String, address:String, description:String, password:String){
        ...
        storageRef.putData(data as Data, metadata: metaData) { (metaData, error) in
            ...
            storageRef.downloadURL(completion: { (url: URL?, error: Error?) in
                ...
                self.getExistingUser(name: name, email: email, age: age, password: password, address: address, description: description, profile: self.imageURL)
