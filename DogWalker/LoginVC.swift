    ...
    @IBAction func btnLoginTapped(_ sender: UIButton) {
        ...
        if error.isEmpty {
            ...
            if self.txtEmail.text?.trim() == "Admin@gmail.com" && self.txtPassword.text?.trim() == "Admin@1234" && self.userType == .Admin{
                ...
                self.loginWalkerUser(email: self.txtEmail.text?.trim() ?? "", password: self.txtPassword.text?.trim() ?? "", userType: self.userType?.getname().description ?? "admin")
            }else if self.userType == .Owner{
                self.loginUser(email: self.txtEmail.text?.trim() ?? "", password: self.txtPassword.text?.trim() ?? "", userType: self.userType?.getname().description ?? "admin")
            }else{
        ...
        _ = AppDelegate.shared.database.collection(type).whereField(dUser_email, isEqualTo: email).whereField(dPassword, isEqualTo: password).whereField(dType, isEqualTo: userType).addSnapshotListener{ querySnapshot, error in
            ...
            if snapshot.documents.count != 0 {
                ...
                if  let name: String = data1[dUser_name] as? String,
                    ...
                    let reserved: Bool = data1[dReserved] as? Bool
                {
                if isEnable {
                    GFunction.userWalker = UserWalkerModel(docID: docId, name: name, experience: exp, email: email, password: password, hourlyRate: hourlyRate, userType: type, timing: timing, from: from, to: to, isEnable: isEnable, description: description,lat: lat,lng: lng,profile: profile, rating: rating,reserved: reserved)
        ...
        _ = AppDelegate.shared.database.collection(type).whereField(dUser_email, isEqualTo: email).whereField(dUser_password, isEqualTo: password).addSnapshotListener{ querySnapshot, error in
            ...
            if snapshot.documents.count != 0 {
                ...
                if  let name: String = data1[dUser_name] as? String,
                    ...
                    let isReserved: Bool = data1[dIsReserved] as? Bool
                {
                if isEnable {
                    GFunction.userOwner = UserOwnerModel(docID: docId, name: name, email: email, password: password, userType: userType, isEnable: isEnable, description: description, profile: profile, address: address, age: age,isReserved: isReserved)
                    GFunction.shared.firebaseRegister(data: email, password: password)
                    Alert.shared.showAlert(message: "Success", completion: nil)
                }else{
                    Alert.shared.showAlert(message: "Please contact to admin !!!", completion: nil)
