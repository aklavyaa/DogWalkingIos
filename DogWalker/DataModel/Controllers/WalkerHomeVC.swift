...
extension WalkerHomeVC: UITableViewDelegate,UITableViewDataSource {
    ...
    func getData(){
        AppDelegate.shared.database.collection(dDogWalker).document(GFunction.userWalker.docID).collection(dOwnerRequest).addSnapshotListener{querySnapshot , error in
